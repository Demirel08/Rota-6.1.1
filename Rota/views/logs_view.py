...
class LogsView(QWidget):
    ...
    def setup_ui(self):
        ...
        columns = ["TARİH / SAAT", "PERSONEL", "İSTASYON", "DURUM", "SİPARİŞ KODU", "MÜŞTERİ", "M²", "ADET"]
        ...
        # M² ve ADET sütunlarını sabit genişlik yap
        header_obj.setSectionResizeMode(6, QHeaderView.Fixed)  # M²
        header_obj.setSectionResizeMode(7, QHeaderView.Fixed)  # ADET
        self.table.setColumnWidth(6, 80)  # M² sütunu
        self.table.setColumnWidth(7, 70)  # ADET sütunu
    ...
    def fill_table(self, data):
        ...
        for row_idx, item in enumerate(data):
            ...
            # 7. M² (toplam m² / sipariş adeti × işlem adeti)
            try:
                total_m2 = float(item.get('declared_total_m2', 0))  # Sipariş toplam m²
                order_qty = float(item.get('quantity', 1))  # Sipariş toplam adet
                processed_qty = float(item.get('processed_quantity', 0))  # Bu işlemde kaç adet

                if order_qty > 0 and total_m2 > 0:
                    # Birim m² hesapla ve işlem adediyle çarp
